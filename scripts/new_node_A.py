	...
	while not rospy.is_shutdown():
		...
		if Input1 == 'Change' :
			...
			if have_goal:
				...
				input_x1 = input('Enter a set for the new x_coordinate:')
				input_y1 = input('Enter a set for the new y_coordinate:')
				
				

				# Convert the input string into a list of numbers
				input_x = [int(x) for x in input_x1.split()]
				input_y = [int(x) for x in input_y1.split()]
				
				
				
				for i in input_x:
				
					rospy.set_param('/des_pos_x' , input_x[i])
					rospy.set_param('/des_pos_y' , input_y[i])
					goal_new.target_pose.pose.position.x = input_x[i]
					goal_new.target_pose.pose.position.y = input_y[i]
					
					goal_status = client.get_state()
				
					while goal_status is not GoalStatus.SUCCEEDED:
					
						
				
						rospy.logwarn("On the %d-th iteration", i)
